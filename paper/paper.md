...
While PyMilo may import functions from widely used scientific libraries during deserialization to restore model behavior (for example NumPy or SciPy), the JSON representation itself never contains executable code; any residual risk is therefore inherited from those already trusted dependencies rather than introduced by the serialization format. Future releases will further restrict this mechanism through a clearly defined whitelist of importable functions and, ultimately, a dedicated transparent function serializer that removes the need for arbitrary imports.

...
class EnsembleModelChain(AbstractChain):
    ...
    def get_transporter(self, model):
        ...
        if isinstance(model, str):
            if model.upper() == "ENSEMBLE":
                return "ENSEMBLE", self.transport
        if self.is_supported(model):
            return "ENSEMBLE", self.transport
        else:
            return get_concrete_transporter(model)

    def serialize_possible_ml_model(self, possible_ml_model):
        """
        ...
        ml_category, transporter = self.get_transporter(possible_ml_model)
    ...
    def deserialize_possible_ml_model(self, possible_serialized_ml_model):
        """
        ...
        if check_str_in_iterable("pymilo-inner-model-type", possible_serialized_ml_model):
            _, transporter = self.get_transporter(possible_serialized_ml_model["pymilo-ml-category"])
            ...
            if isinstance(value, list):
                    ...
                    if isinstance(item, tuple):
                        ...
                        for inner_idx, inner_item in enumerate(listed_tuple):
                            if pt.is_preprocessing_module(inner_item):
                                ...
                                has_inner_model, result = self.serialize_possible_ml_model(inner_item)
                                if has_inner_model:
                                    has_inner_tuple_with_ml_model = True
                                listed_tuple[inner_idx] = result
                        value[idx] = listed_tuple
                    else:
                        value[idx] = self.serialize_possible_ml_model(item)[1]
                ...
                if check_str_in_iterable("pymilo-bunch", value):
                    ...
                    for inner_key, inner_value in value["pymilo-bunch"].items():
                        new_value[inner_key] = self.serialize_possible_ml_model(inner_value)[1]
                ...
                has_inner_model, result = self.serialize_models_in_ndarray(value)
                if has_inner_model:
                    ensemble_object.__dict__[key] = result

            else:
                ensemble_object.__dict__[key] = self.serialize_possible_ml_model(value)[1]
    ...
    def serialize_models_in_ndarray(self, ndarray_instance):
        """
        ...
        for item in ndarray_instance:
            if isinstance(item, ndarray):
                has_inside_model, result = self.serialize_models_in_ndarray(item)
                ...
                has_ml_model, result = self.serialize_possible_ml_model(item)
                if has_ml_model:
                    new_list.append(result)
                else:
                    has_inner_model = False
    ...
    def deserialize_models_in_ndarray(self, serialized_ndarray):
        """
        ...
        for _, item in enumerate(inner_list):
            if gdst.is_deserialized_ndarray(item):
                has_inside_model, result = self.deserialize_models_in_ndarray(item)
                if not has_inside_model:
                ...
                has_ml_model, result = self.deserialize_possible_ml_model(item)
                if has_ml_model:
                    new_list.append(result)
                else:
                    has_inner_model = False
    ...
    def deserialize(self, ensemble, is_inner_model=False):
        ...
        for key, value in data.items():
                ...
                if check_str_in_iterable("pymiloed-data-structure",
                    ...
                    for listed_tuple in listed_tuples:
                        ...
                        retrieved_model = pt.deserialize_pre_module(serialized_model) if pt.is_preprocessing_module(
                            serialized_model) else self.deserialize_possible_ml_model(serialized_model)[1]
                    ...
                    has_inner_model, result = self.deserialize_models_in_ndarray(value)
                    if has_inner_model:
                        data[key] = result
            ...
            has_ml_model, result = self.deserialize_possible_ml_model(value)
            if has_ml_model:
                data[key] = result

...
def get_preprocessed_before_deserialization(
        model_type,
        unserializable_key,
        unserializable_content):
    if (unserializable_key not in KEYS_NEED_PREPROCESSING_BEFORE_DESERIALIZATION.keys()):
    ...
    match unserializable_key:
        ...
        case "_base_loss":
            return get_deserialized_base_loss(
                model_type, unserializable_content)
...
def get_sklearn_data(model):
    ...
    for key in data.keys():
        ...
        # Handling loss functions, no abstract generic type for these
        # losses(SGD based models)
        elif (
        ...
        # list type which containts unserializable type numpy.int32
        ...
        # object of  unserializable LabelBinarizer class
        elif isinstance(data[key], preprocessing.LabelBinarizer):
            data[key] = get_serialized_label_binarizer(data[key])

        # object of  unserializable numpy.ndarray class
    ...
    for key in data.keys():
        if (isinstance(data[key], dict)):
            if ("np-type" in data[key].keys()):
                data[key] = NUMPY_TYPE_DICT[data[key]
                                            ["np-type"]](data[key]["value"])

        if (key in KEYS_NEED_PREPROCESSING_BEFORE_DESERIALIZATION.keys()):
            data[key] = get_preprocessed_before_deserialization(
                import_obj.type, key, data[key])

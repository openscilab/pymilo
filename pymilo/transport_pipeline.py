...
from .chain import NumpyOrGeneralDataStructureTransporter
from .chain import BaseLossObjectTransporter
from .chain import LossFunctionTransporter
from .chain import LabelBinarizerTransporter

from .pymilo_param import SKLEARN_MODEL_TABLE
from .utils.util import get_sklearn_type, is_primitive, is_iterable

linear_model_transport_pipeline = {
    "NumpyOrGeneralDataStructureTransporter": NumpyOrGeneralDataStructureTransporter(),
    "BaseLossObjectTransporter": BaseLossObjectTransporter(),
...
def transport_linear_model(request, command, is_inner_model = False):
    ...
    if(command == Command.SERIALIZE):
        ...
        for transporter in linear_model_transport_pipeline.keys():
                linear_model_transport_pipeline[transporter].transport(request,Command.SERIALIZE)
        ...
        if(is_inner_model):
            ...
            raw_model = SKLEARN_MODEL_TABLE[request.type]()
            data = request.data
        # first deserializing the inner linear models(one depth inner linear models have been deserialized -> TODO full depth).
        ...
        for transporter in linear_model_transport_pipeline.keys():
                linear_model_transport_pipeline[transporter].transport(request,Command.DESERIALZIE, is_inner_model)

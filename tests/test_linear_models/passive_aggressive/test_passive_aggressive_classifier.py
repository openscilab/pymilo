...
def test_passive_aggressive_classifier():
    ...
    pac_random_state = 0
    pac_tol = 1e-3
    passive_aggressive_classifier = PassiveAggressiveClassifier(max_iter= pac_max_iter, random_state= pac_random_state, tol= pac_tol)
    # Train the model using the training sets
    passive_aggressive_classifier.fit(x_train, y_train)
    return test_pymilo_classification(passive_aggressive_classifier, MODEL_NAME, (x_test,y_test))
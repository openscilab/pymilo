...
def test_multi_task_lasso_cv():
    ...
    multi_task_lasso_cv = MultiTaskLassoCV(random_state=lasso_random_state,alphas=lasso_alphas,cv= lasso_cv)
    # Train the model using the training sets
    multi_task_lasso_cv.fit(x_train, y_train)
    return test_pymilo_regression(multi_task_lasso_cv, MODEL_NAME, (x_test,y_test))